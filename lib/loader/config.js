...
class Config extends LoaderInterface {
  load () {
    const { appDir, root, env } = this.app
    const configs = {
    ...
    this.writeRuntimeFile(path.join('config', `${root}.json`), configs[root])

    const { bootstraps = [] } = configs[root]
    const modules = this._parseBootstraps(bootstraps, appDir)
    ...
    for (let key in modules) {
      ...
      this.writeRuntimeFile(path.join('config', `${key}.json`), configs[key])
    }
  ...
  _parseBootstraps (bootstraps, appDir) {
    const modules = {}
    for (let bootstrap of bootstraps) {
      if (typeof bootstrap === 'string') {
        modules[bootstrap] = path.join(appDir, bootstrap)
