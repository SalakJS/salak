...
class Router extends LoaderInterface {
  load () {
    ...
    const { prefix, loadOrder, defaultRoute, defaultMethods, replaceIndex = true } = this._parseRouteConf(this.app.config('routes'))
  ...
  _parseRouteConf (route = {}, isModule = false) {
    const { prefix, load, defaultRoute, defaultMethods, replaceIndex, alias } = route
    let defaultAction
