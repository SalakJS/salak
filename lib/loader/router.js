...
class Router extends LoaderInterface {
  load () {
    ...
    const { prefix, loadOrder, defaultRoute, defaultMethods, replaceIndex = true, ignoreValidateResponses } = this._parseRouteConf(this.app.config('routes'))
    ...
    this.writeRuntimeFile(path.join('router', 'definitions.json'), this._removeJoiValidate(routesDefinitions))
  ...
  [constants.MODE_SINGLE] (rootRouter, routesDefinitions, options) {
    const { loadOrder, defaultRoute, defaultMethods, replaceIndex } = options
    ...
    const routesFactory = this._addModuleRoutes(rootRouter, this.app.root, {
      ...
      setDefaultRootRoute: defaultRoute && (({
        ...
        const methods = options.defaultMethods || constants.DEFAULT_METHODS
        this.app.logger.debug(`Add root default Route: / method: ${methods} action: ${this.app.root}-${controllerName}-${action}`)
        fn({
          rootRouter,
          defaultMethods: methods,
          moduleMiddlewares: null
        })
      })
    })
  ...
  [constants.MODE_MULTIPLE] (rootRouter, routesDefinitions, options) {
    ...
    for (let key in modules) {
      ...
      const router = new SalakRouter({
        prefix,
        ignoreValidateResponses: ignoreValidateResponses !== undefined ? ignoreValidateResponses : options.ignoreValidateResponses
      })
      ...
      routesDefinitions.modules.push({
        name: this._parseCompletedPath(routeModuleName, prefix),
        routes: routesFactory
      })
  ...
  _addModuleRoutes (router, module, {
    ...
    const routesFactory = []
    const keys = loadOrder && loadOrder.length ? loadOrder : Object.keys(controllerObjs)
    this.app.logger.debug(`Route Order: ${keys}`)
    for (let controllerName of keys) {
      ...
      const { routes = {}, rules = {} } = this._parseBehavior(behaviorObjs[controllerName], Controller, Object.keys(actions), module)
        ...
        for (let pathKey in item) {
          ...
          const handler = async (ctx, next) => {
            ...
            const controllerMiddlewares = [async (ctx, next) => {
              // add controller info to ctx
              ctx.routeInfo = {
                module,
                controller: controllerName,
                action: actionMeta.key,
                key: key
              }
              await next()
            }]
  ...
  _parseBehavior (Behavior, Controller, keys = [], module) {
    const routes = {}
    const rules = {}
    ...
    if (is.class(Behavior)) {
      ...
      for (let key of keys) {
        const behaviorKey = `action${key[0].toUpperCase() + key.slice(1)}`
        if (!is.function(behavior[behaviorKey])) {
          continue
        }

        rules[key] = behavior[behaviorKey]()
      }
    }
  ...
  _parseRouteConf (route = {}, isModule = false) {
    const { prefix, load, defaultRoute, defaultMethods, replaceIndex, alias, ignoreValidateResponses } = route
    let defaultAction
  ...
  _parseRoutes (actions, routes, rules, defaultMethods = constants.DEFAULT_METHODS) {
    ...
    for (let key in actions) {
      ...
      for (let item in routes) {
        ...
        ret[key][routePath] = {
          method: is.array(routeMethod) ? routeMethod : [routeMethod],
          meta,
          validate
        }
        hasSpecified = true
