...
class Salak extends Koa {
   ...
   * @param {String} options.opts.app - app directory name, default ``
  ...
  constructor ({ baseDir = process.cwd(), opts } = {}) {
    ...
    this.appDir = path.join(this.baseDir, opts.app)
    this.root = opts.root
    ...
    this.logger.app.info(`Salak version: ${this.version}`)
    this.logger.app.info(`Enviroment: ${this.env}`)
  ...
  async run (port = process.env.PORT || this.config('port') || 3000) {
    ...
    server.on('listening', () => {
      this.logger.app.info(`Server running at ${this.config('host') || 'http://127.0.0.1'}:${port}`)
